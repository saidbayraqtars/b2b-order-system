...
// Draft order lines, scoped to one company. A sales rep serves several customers
// in a day, so switching company clears the cart instead of mixing prices —
// pricing is company-specific and a line is only valid for the company it came from.
...
interface CartState {
  ...
  lines: CartLine[];
  /** Point the cart at a company; drops lines belonging to a different one. */
...
export const useCart = create<CartState>((set) => ({
  ...
  setCompany: (companyId) =>
    set((state) =>
      state.companyId === companyId ? state : { companyId, lines: [] },
    ),

  add: (seed, qty) =>
    set((state) => {
      const existing = state.lines.find((l) => l.variantId === seed.variantId);
      ...
      return {
        lines: [...state.lines, { ...seed, quantity: normalizeQty(seed, startQty) }],
      };
    }),
  ...
  setQty: (variantId, qty) =>
    set((state) => ({
      lines: state.lines.map((l) =>
        l.variantId === variantId ? { ...l, quantity: normalizeQty(l, qty) } : l,
      ),
    })),
